// Translation dictionaries for the site's 3 supported languages. Structure
// mirrors the pages/components that consume it (nav, footer, home, ...).
// Values can be strings OR arrays/objects of strings - useTranslation()'s
// t() just returns whatever is stored at that path, so callers that need a
// list (FAQ, steps, ...) get the list back typed as `any`.
//
// NOTE ON SCOPE: this covers every public/marketing page (Navbar, Footer,
// Home, Services chrome, About, Contact, Blog, Privacy, Terms) plus shared
// widgets (DiscountBanner, MobileBookingBar, FloatingContact, TeamPreview,
// PriceEstimator, NotFound). Authenticated app screens (Booking form,
// Login/Register, Kabinet/Dashboard, Admin panel) are NOT translated in this
// pass and stay in Uzbek - they're used post-signup by a mostly-local
// audience, and translating transactional forms + validation copy is a
// separate, larger effort. Ask if you want those covered too.

export type Lang = 'uz' | 'en' | 'ru'

export const LANGUAGES: { code: Lang; label: string }[] = [
  { code: 'uz', label: "O'zbek" },
  { code: 'en', label: 'English' },
  { code: 'ru', label: 'Русский' },
]

const translations = {
  uz: {
    brand: {
      name: 'Prime Standard & Co',
      tagline: 'Tozalash xizmati',
      initials: 'PS',
    },
    nav: {
      home: 'Bosh sahifa',
      services: 'Xizmatlar va narxlar',
      blog: 'Maslahatlar',
      about: 'Biz haqimizda',
      contact: 'Aloqa',
      login: 'Kirish',
      myAccount: 'Kabinetim',
      signOut: 'Chiqish',
      bookNow: 'Buyurtma berish',
      adminPanel: 'Admin panel',
      menu: 'Menyu',
      toggleTheme: 'Mavzuni almashtirish',
      language: 'Til',
    },
    footer: {
      tagline:
        "Uy va ofislaringiz uchun ishonchli, tez va sifatli tozalash xizmati. Onlayn band qiling — Payme yoki Click orqali xavfsiz to'lang.",
      servicesTitle: 'Xizmatlar',
      services: ['Standart tozalash', 'Chuqur tozalash', "Ko'chishdan oldin/keyin", 'Ofis tozalash', 'Qurilishdan keyingi tozalash'],
      companyTitle: 'Kompaniya',
      about: 'Biz haqimizda',
      blog: 'Maslahatlar',
      contact: 'Aloqa',
      bookNow: 'Buyurtma berish',
      contactTitle: 'Aloqa',
      address: "Toshkent va Samarqand, O'zbekiston",
      rights: 'Barcha huquqlar himoyalangan.',
      privacy: 'Maxfiylik siyosati',
      terms: 'Foydalanish shartlari',
    },
    discountBanner: {
      pre: 'Birinchi buyurtmangizga',
      percent: '-20%',
      mid: '— band qilishda izohga',
      code: 'YANGI20',
      post: "promo-kodini yozing.",
      cta: 'Hoziroq band qiling',
      close: 'Yopish',
    },
    mobileBar: {
      call: "Qo'ng'iroq qilish",
      book: 'Band qilish',
    },
    floatingContact: {
      whatsapp: 'WhatsApp orqali yozing',
      telegram: 'Telegram orqali yozing',
      aiChat: 'AI yordamchi bilan suhbat',
      close: "Bog'lanishni yopish",
      open: 'Biz bilan bog\'laning',
      back: 'Orqaga',
      aiTitle: 'AI yordamchi',
      aiGreeting:
        "Salom! Men Prime Standard & Co'ning AI yordamchisiman. Xizmatlar, narxlar yoki band qilish haqida savolingiz bo'lsa, yozing.",
      aiUnavailable:
        "AI yordamchi hozircha mavjud emas. Savolingiz bo'lsa, WhatsApp yoki telefon orqali to'g'ridan-to'g'ri yozing - pastdagi menyudan tanlashingiz mumkin.",
      aiPlaceholder: 'Savolingizni yozing…',
      aiSending: 'Yozmoqda…',
      aiError: "Kechirasiz, javob bera olmadim. Qaytadan urinib ko'ring yoki WhatsApp orqali yozing.",
      aiSend: 'Yuborish',
    },
    home: {
      heroTag: 'Toshkent va Samarqanddagi tozalash xizmati',
      heroTitle: 'Uyingiz yoki ofisingiz uchun',
      heroTitleHighlight: 'professional tozalash',
      heroDesc:
        "Onlayn band qiling, Payme yoki Click orqali xavfsiz to'lang va tekshirilgan xizmatchimiz eshigingizga keladi.",
      ctaBook: 'Hoziroq band qilish',
      ctaPrices: "Narxlarni ko'rish",
      heroTrust1: 'Tekshirilgan xizmatchilar',
      heroTrust2: "Xavfsiz onlayn to'lov",
      heroTrust3: 'Tez javob beramiz',
      socialProofBadge: '4.9/5 (1,200+ xonadonlar ishonchi)',
      instantQuoteTitle: 'Bir zumda narx hisoblash',
      roomsLabel: 'Xonalar soni',
      typeLabel: 'Tozalash turi',
      standardTitle: 'Standart',
      standardSub: 'Kundalik tozalik',
      deepTitle: 'Mukammal (Deep)',
      deepSub: 'Chuqur tozalash',
      estimatedPriceLabel: 'Taxminiy narx:',
      bookNowBtn: 'Bron qilish →',
      pillar1Title: 'Tekshirilgan xodimlar',
      pillar1Desc: '100% shaxsiyati tekshirilgan mutaxassislar',
      pillar2Title: 'Qoniqish kafolati',
      pillar2Desc: 'Yoqmasa, qayta bepul tozalab beramiz',
      pillar3Title: 'Shaffof narxlar',
      pillar3Desc: 'Hech qanday yashirin komissiyalarsiz',
      resultsTag: 'Real Natijalar',
      resultsTitle: 'Oldin va Keyin Taqqoslash',
      resultsDesc: "Slayderni surib tozalash sifatiga o'zingiz baho bering",
      beforeLabel: 'Tozalashdan oldin',
      afterLabel: 'Tozalashdan keyin',
      plansTag: 'Shaffof Tariflar',
      plansTitle: 'Mos Tarifni Tanlang',
      plansDesc: 'Hech qanday yashirin komissiyalarsiz kafolatlangan narxlar',
      priceFrom: '/dan',
      selectBtn: 'Tanlash',
      bookPlanBtn: 'Hoziroq bron qilish',
      mostPopularBadge: '✨ ENG MASHHUR (MOST POPULAR)',
      planStandardName: 'Standart tozalash',
      planStandardDesc: 'Kundalik tozalik va tartib uchun ideal',
      planStandardF1: 'Pol va gilam yuvish',
      planStandardF2: 'Changlarni artish',
      planStandardF3: 'Oshxona va hammom dezinfeksiyasi',
      planDeepName: 'Mukammal tozalash (Deep Clean)',
      planDeepDesc: "Chuqur kir va qatlamlardan xalos bo'lish",
      planDeepF1: 'Standart tozalashdagi barcha amallar',
      planDeepF2: 'Mebellar orti va qiyin joylar',
      planDeepF3: "Kafel choklari va yog' dog'larini yo'qotish",
      planDeepF4: 'Derazalarni ichki/tashqi yuvish',
      planMoveName: "Ko'chib kirish / chiqish",
      planMoveDesc: "Yangi uyga ko'chish yoki ta'mirdan so'ng",
      planMoveF1: "To'liq tozalash va zararsizlantirish",
      planMoveF2: 'Barcha shkaflar va tortmalarni ichidan yuvish',
      planMoveF3: 'Qurilish va bo\'yoq qoldiqlarini tozalash',
      verifiedCustomer: 'Tasdiqlangan mijoz',
      verifiedServiceMeta: 'Xonadon tozalash · Toshkent',
      trustPoints: [
        { label: 'Tekshirilgan xizmatchilar', desc: "Har biri hujjat/pasport orqali tekshiriladi, so'ng shaxsan suhbatdan o'tadi" },
        { label: "Xavfsiz onlayn to'lov", desc: 'Payme va Click orqali' },
        { label: 'Tez javob', desc: "So'rovga bir necha daqiqada javob" },
        { label: 'Shaffof narx', desc: "Yashirin to'lovlarsiz, oldindan aniq" },
      ],
      reviewsCount: 'sharh',
      reviewsTitle: 'Mijozlarimiz fikri',
      reviewsDesc: "Haqiqiy mijozlarimizdan olingan sharhlar — soxta yoki sotib olingan sharh yo'q.",
      areaTitle: "Toshkent va Samarqand shaharlari bo'ylab xizmat ko'rsatamiz",
      areaDesc: "Manzilingiz qaysi tumanda bo'lishidan qat'iy nazar, xizmatchimiz belgilangan vaqtda yetib boradi.",
      referralTitle: "Do'stingizni taklif qiling — ikkalangiz ham chegirma oling",
      referralDesc: "Do'stingiz sizning havolangiz orqali ro'yxatdan o'tsa, u birinchi buyurtmasiga chegirma oladi, siz esa kredit ishlab olasiz.",
      referralButton: 'Batafsil',
      howTitle: 'Qanday ishlaydi',
      howDesc: 'Uch oddiy qadamda uyingiz yoki ofisingizni tozalatib oling.',
      steps: [
        { title: '1. Xizmatni tanlang', desc: "Uy yoki ofis, tozalash turi va qo'shimcha xizmatlarni belgilang." },
        { title: '2. Vaqtni band qiling', desc: 'Sizga qulay sana va soatni tanlang — bir martalik yoki muntazam.' },
        { title: "3. Onlayn to'lang", desc: "Payme yoki Click orqali xavfsiz to'lov qiling, xizmatchi tayinlanadi." },
      ],
      estimateTitle: 'Narxni hoziroq bilib oling',
      estimateDesc: "Ro'yxatdan o'tmasdan turib, taxminiy narxni ko'ring.",
      servicesTitle: 'Xizmatlarimiz',
      servicesDesc: 'Ehtiyojingizga mos xizmat turini tanlang — narx band qilishda avtomatik hisoblanadi.',
      serviceTiles: [
        {
          title: 'Standart tozalash',
          desc: 'Kundalik tozalik — chang tozalash, sanuzel va oshxona.',
          img: 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=800&q=80',
        },
        {
          title: 'Chuqur tozalash',
          desc: "Ko'chishdan oldin/keyin yoki mavsumiy chuqur tozalash.",
          img: 'https://images.unsplash.com/photo-1558317374-067fb5f30001?auto=format&fit=crop&w=800&q=80',
        },
        {
          title: 'Ofis tozalash',
          desc: 'Ish joyingiz uchun muntazam yoki bir martalik tozalash.',
          img: 'https://images.unsplash.com/photo-1527515637462-cff94eecc1ac?auto=format&fit=crop&w=800&q=80',
        },
        {
          title: 'Qurilishdan keyingi tozalash',
          desc: "Yangi qurilgan ko'p qavatli uylar va ofislar uchun to'liq remontdan keyingi tozalash.",
          img: 'https://images.unsplash.com/photo-1628177142898-93e36e4e3a50?auto=format&fit=crop&w=800&q=80',
        },
      ],
      detail: 'Batafsil',
      whyTag: 'Nega Prime Standard & Co',
      whyTitle: 'Ishonchli, shaffof va qulay',
      whyUs: [
        "Har bir xizmatchi ro'yxatga olinishdan oldin hujjat/pasporti tekshiriladi, shaxsan suhbatdan o'tadi va tajribasi tekshiriladi",
        "Narx band qilishdan oldin aniq hisoblanadi — xona soni yoki maydonga qarab, yashirin qo'shimchalarsiz",
        "To'lov faqat tasdiqlangan xavfsiz kanallar — Payme va Click orqali amalga oshiriladi",
        'Har bir buyurtma holati shaxsiy kabinetingizda real vaqtda kuzatiladi',
        'Xizmat ko\'rsatish jarayonida mulkingizga zarar yetkazilsa, kompaniya moddiy javobgarlikni o\'z zimmasiga oladi',
      ],
      teamTitle: 'Jamoamiz bilan tanishing',
      teamDesc: "Har bir xizmatchimiz shaxsan suhbatdan o'tgan va tajribasi tekshirilgan.",
      teamExperience: 'yil tajriba',
      teamSeeAll: 'Butun jamoani ko\'rish →',
      galleryTitle: 'Natija qanday ko\'rinadi',
      galleryDesc: 'Toza va tartibli xona — har bir tashrifdan kutadigan natijangiz shu.',
      faqTag: 'Savol-javob',
      faqTitle: "Ko'p so'raladigan savollar",
      faqs: [
        {
          q: "Xodimlar o'zlari bilan tozalash vositalarini olib kelishadimi?",
          a: "Ha, mutaxassislarimiz to'liq professional tozalash vositalari, mikrofibra matolar va zamonaviy uskunalar bilan yetib kelishadi. Sizdan hech qanday qo'shimcha narsa talab etilmaydi.",
        },
        {
          q: "Tozalash vaqtida uyda bo'lishim shartmi?",
          a: "Shart emas. Xizmatchimizga kalitni topshirib o'z ishlaringiz bilan shug'ullanishingiz mumkin. Tozalash tugashidan 30 daqiqa oldin sizga qo'ng'iroq qilib xabar beramiz.",
        },
        {
          q: "Agar biror narsa sinib qolsa yoki zarar yetsa nima bo'ladi?",
          a: "Barcha xizmatchilarimiz shaxsan va moddiy javobgarlik shartnomasi bilan ishlaydi. Qandaydir ko'ngilsizlik yuz bersa, kompaniyamiz yetkazilgan zararni 100% qoplab beradi.",
        },
        {
          q: "Xizmatchilar qanday tekshiriladi?",
          a: "Har bir xizmatchi ishga qabul qilinishdan oldin shaxsan suhbatdan o'tadi, huquqni muhofaza qilish organlari bazasi va sobiq ish joylaridan tavsiflar tekshiriladi.",
        },
        {
          q: "To'lovni qachon va qanday amalga oshiraman?",
          a: "Buyurtma berish jarayonida Payme, Click yoki naqd pulda to'lashingiz mumkin. Yashirin komissiyalar yo'q — faqat kelishilgan narx to'lanadi.",
        },
      ],
      ctaTitle: 'Bugun tozalash xizmatini band qiling',
      ctaDesc: 'Narxni bir necha soniyada hisoblang va onlayn to\'lang.',
      ctaButton: 'Buyurtma berish',
    },
    services: {
      heroTitle: 'Xizmatlar va narxlar',
      heroDesc:
        "Narxlarimiz AQSHdagi yetakchi tozalash platformalari (Merry Maids, Molly Maid, MaidPro) tuzilmasi asosida shakllantirilgan va O'zbekiston bozoriga moslashtirilgan. Aniq narx xona soni yoki maydonga qarab band qilish sahifasida avtomatik hisoblanadi.",
      loading: 'Yuklanmoqda…',
      home: 'Uy',
      office: 'Ofis',
      startingPrice: "boshlang'ich narx (1 xona)",
      perExtraRoom: "har bir qo'shimcha xona uchun",
      min: 'min.',
      bookThis: 'Ushbu xizmatni band qilish',
      tariffsTitle: 'Tarif darajalari va misol narxlar',
      tariffsDesc: "Ehtiyojingizga qarab o'zingizga mos tarif darajasini tanlang:",
      example1Room: 'Standart 1-xonali tozalash misolida:',
      teamTitle: 'Jamoamiz',
      teamDesc: "Har bir xizmatchimiz tekshirilgan va sinovdan o'tgan.",
      yearsExperience: 'yil tajriba',
    },
    about: {
      title: 'Biz haqimizda',
      intro:
        "Prime Standard & Co — Toshkentda faoliyat yurituvchi zamonaviy tozalash xizmati. Maqsadimiz uy va ofislarni onlayn bir necha bosim bilan, tekshirilgan xizmatchilar yordamida va xavfsiz onlayn to'lov orqali tozalatish imkonini berish.",
      pillars: [
        { title: 'Tekshirilgan xizmatchilar', desc: "Har biri hujjat/pasporti tekshiriladi, so'ng shaxsan suhbatdan o'tadi" },
        { title: "Xavfsiz to'lov", desc: 'Payme, Click yoki chek orqali' },
        { title: 'Ekologik vositalar', desc: "Sog'liq uchun xavfsiz tozalash kimyoviy vositalari" },
      ],
      whyTitle: 'Nega aynan biz?',
      reasons: [
        "Barcha xizmatchilar ishga qabul qilinishdan oldin hujjat/pasport tekshiruvidan va sinovdan o'tadi",
        'Payme va Click orqali xavfsiz onlayn to\'lov',
        "Shaffof narxlash — yashirin to'lovlar yo'q",
        'Ekologik va xavfsiz tozalash vositalari',
        "Xizmat paytida yetkazilgan zarar uchun moddiy kafolat beramiz",
      ],
    },
    contact: {
      title: 'Aloqa',
      desc: "Savollaringiz bo'lsa, biz bilan bog'laning.",
      sentMessage: 'Xabaringiz uchun rahmat! Tez orada bog\'lanamiz.',
      nameLabel: 'Ism',
      contactLabel: 'Telefon yoki email',
      messageLabel: 'Xabar',
      sending: 'Yuborilmoqda…',
      send: 'Yuborish',
      error: "Xabar yuborilmadi. Qaytadan urinib ko'ring.",
      address: "Toshkent, O'zbekiston",
    },
    privacy: {
      title: 'Maxfiylik siyosati',
      updated: "Oxirgi yangilanish: 2026-yil avgust",
      sections: [
        {
          h: "1. Qanday ma'lumot to'playmiz",
          p: "Ro'yxatdan o'tganingizda yoki buyurtma berganingizda ism, telefon raqam, email va manzil kabi ma'lumotlarni to'playmiz. Bu ma'lumotlar faqat xizmatni ko'rsatish — buyurtmani tasdiqlash, xizmatchini tayinlash va siz bilan bog'lanish uchun ishlatiladi.",
        },
        {
          h: "2. Ma'lumotlardan qanday foydalanamiz",
          p: "Ma'lumotlaringiz uchinchi shaxslarga sotilmaydi. Ular faqat buyurtmani bajarish (xizmatchiga biriktirish), to'lovni amalga oshirish (Payme, Click) va zarurat tug'ilganda siz bilan bog'lanish uchun ishlatiladi.",
        },
        {
          h: "3. Ma'lumotlarni saqlash",
          p: "Ma'lumotlar xavfsiz serverlarda saqlanadi va faqat vakolatli xodimlar tomonidan, xizmat ko'rsatish maqsadida ko'riladi.",
        },
        {
          h: '4. Sizning huquqlaringiz',
          p: "Shaxsiy kabinetingizdan o'z ma'lumotlaringizni ko'rishingiz mumkin. Ma'lumotlaringizni o'zgartirish yoki o'chirishni so'rash uchun {email} manziliga murojaat qiling.",
        },
        {
          h: "5. Cookie va texnik ma'lumotlar",
          p: "Saytdan qulay foydalanish uchun brauzeringizda ba'zi sozlamalar (masalan, tanlangan mavzu va til) saqlanishi mumkin. Ular shaxsingizni aniqlash uchun ishlatilmaydi.",
        },
        {
          h: "6. Bog'lanish",
          p: "Maxfiylik siyosati bo'yicha savollaringiz bo'lsa, {email} orqali biz bilan bog'lanishingiz mumkin.",
        },
      ],
    },
    terms: {
      title: 'Foydalanish shartlari',
      updated: "Oxirgi yangilanish: 2026-yil avgust",
      sections: [
        {
          h: '1. Xizmat haqida',
          p: "Prime Standard & Co uy va ofislar uchun tozalash xizmatlarini onlayn band qilish platformasi. Saytdan foydalanish orqali siz ushbu shartlarga rozilik bildirasiz.",
        },
        {
          h: '2. Buyurtma va narxlar',
          p: "Narx xizmat turi, xona soni yoki maydon, tanlangan tarif va qo'shimcha xizmatlarga qarab hisoblanadi va band qilish jarayonida oldindan ko'rsatiladi. Yakuniy narx to'lovdan oldin tasdiqlanadi.",
        },
        {
          h: "3. To'lov",
          p: "To'lovlar Payme yoki Click orqali amalga oshiriladi. To'lov muvaffaqiyatli amalga oshirilgach, buyurtma tasdiqlangan hisoblanadi.",
        },
        {
          h: "4. Bekor qilish va o'zgartirish",
          p: "Buyurtmani bekor qilish yoki sanasini o'zgartirish uchun shaxsiy kabinetingiz orqali murojaat qiling yoki biz bilan telefon orqali bog'laning.",
        },
        {
          h: '5. Javobgarlik',
          p: "Xizmat natijasidan qoniqmagan holatlarda Aloqa sahifasi orqali murojaat qiling — muammoni tezkor hal qilishga harakat qilamiz.",
        },
        {
          h: "6. Shartlarning o'zgarishi",
          p: "Ushbu shartlar vaqti-vaqti bilan yangilanishi mumkin. Muhim o'zgarishlar haqida saytda e'lon qilinadi.",
        },
      ],
    },
    blog: {
      title: 'Maslahatlar',
      desc: "Uy va ofisni toza saqlash bo'yicha qisqa va foydali maslahatlar.",
      readMinutes: 'daqiqa',
      read: "O'qish",
      allPosts: 'Barcha maslahatlar',
      ctaTitle: 'Tozalash xizmati kerakmi?',
      ctaDesc: 'Bir necha daqiqada onlayn band qiling.',
      ctaButton: 'Band qilish',
    },
    notFound: {
      text: 'Sahifa topilmadi.',
      backHome: 'Bosh sahifaga qaytish',
    },
    priceEstimator: {
      title: 'Taxminiy narx',
      desc: "Xizmat turi va o'lchamni tanlang — narxni bir zumda ko'ring.",
      serviceType: 'Xizmatni tanlang',
      area: 'Maydon (m²)',
      rooms: 'Xonalar soni',
      estimateLabel: 'Taxminiy narx (standart tarif)',
      continue: 'Band qilishni davom ettirish',
    },
    pricing: {
      tierLabels: { standard: 'Standart', premium: 'Premium', elite: 'Elite' },
      tierPerks: {
        standard: ['Tajribali xizmatchi', 'Standart tozalash vositalari', 'Belgilangan kunda tashrif'],
        premium: [
          "Katta tajribaga ega xizmatchi",
          'Premium ekologik vositalar',
          "Qo'shimcha sifat nazorati",
          'Vaqtni tanlashda ustuvorlik',
        ],
        elite: [
          "Eng yuqori reytingli jamoa boshlig'i",
          'Premium vositalar + zararsizlantirish',
          '2 bosqichli sifat nazorati',
          'Bir soatlik aniq vaqt oynasi',
          "24/7 ustuvor mijozlarni qo'llab-quvvatlash",
        ],
      },
      frequencyLabels: {
        once: 'Bir martalik',
        weekly: 'Har hafta (-20%)',
        biweekly: 'Ikki haftada bir (-15%)',
        monthly: 'Oyiga bir marta (-10%)',
      },
      firstBookingDiscountLabel: 'Birinchi buyurtma chegirmasi (-10%)',
      referralDiscountLabel: 'Referral chegirmasi (-20%)',
      repairTierLabels: { standard: 'Kosmetik', premium: 'Standart', elite: 'Evroremont' },
      repairTierPerks: {
        standard: [
          "Byudjet materiallar (mijoz ta'minlashi ham mumkin)",
          'Tez bajarish (1-2 hafta)',
          "Asosiy ishlar: bo'yash, shpaklash, kichik ta'mirlar",
        ],
        premium: [
          "O'rta darajadagi sifatli materiallar",
          'Bosqichma-bosqich sifat nazorati',
          '1 yillik kafolat',
          "Loyihaviy smeta va grafik",
        ],
        elite: [
          'Premium import materiallar',
          "Dizayner konsultatsiyasi va 3D loyiha",
          "Shaxsiy loyiha menejeri biriktiriladi",
          '3 yillik kafolat',
          'Ishdan keyingi tozalash xizmati bepul',
        ],
      },
    },
    booking: {
      title: 'Xizmatni band qilish',
      subtitle: "Ma'lumotlarni to'ldiring — narx pastda avtomatik hisoblanadi.",
      loading: 'Yuklanmoqda…',
      firstBookingText: "Birinchi buyurtmangizga 20% chegirma qo'llandi!",
      referralAppliedText: "Referral kodingiz orqali 15% chegirma qo'llandi!",
      categoryLabel: "Yo'nalish",
      categoryCleaning: 'Tozalash',
      categoryRepair: "Ta'mirlash",
      serviceType: 'Xizmatni tanlang',
      propertyType: 'Mulk turi',
      home: 'Uy / kvartira',
      office: 'Ofis',
      area: 'Maydon (m²)',
      rooms: 'Xonalar soni',
      floor: 'Necha qavatda',
      floorHelp: "Qavat qancha baland bo'lsa, xizmat narxi shuncha oshadi (ko'tarish/logistika uchun)",
      address: 'Manzil',
      addressPlaceholder: "Ko'cha, uy, kvartira raqami",
      city: 'Shahar',
      frequency: 'Chastota',
      date: 'Sana',
      time: 'Vaqt',
      tier: 'Tarif',
      repairTier: "Ta'mirlash darajasi",
      repairPromo: "Bepul smeta va konsultatsiya — birinchi buyurtmangizga maxsus narx taklif qilamiz.",
      addonsLabel: "Qo'shimcha xizmatlar",
      repairPhotosLabel: 'Loyiha rasmlari (ixtiyoriy)',
      repairPhotosDesc: "Xonaning hozirgi holatini suratga oling — shunda aniqroq baho beramiz. Eng ko'pi 2 ta rasm.",
      repairPhotosAdd: 'Rasm qo\'shish',
      repairPhotosRemove: 'O\'chirish',
      repairNotesLabel: 'Loyiha tavsifi (ixtiyoriy)',
      repairNotesPlaceholder: "Masalan: 2 xonali kvartira, devorlarda nam dog'lari bor, yangi elektr simlari kerak...",
      repairPhotoError: "Rasmni yuklab bo'lmadi. Qaytadan urinib ko'ring yoki boshqa rasm tanlang.",
      firstBookingBanner: 'Bu sizning birinchi buyurtmangiz — jami summaga avtomatik -10% chegirma qo\'llanildi.',
      referralBanner: "Referral kodidan foydalangansiz — birinchi buyurtmangizga avtomatik -20% chegirma qo'llanildi.",
      nameLabel: 'Ism',
      phoneLabel: 'Telefon raqam',
      notesLabel: "Qo'shimcha izoh (ixtiyoriy)",
      priceDetailsTitle: 'Narx tafsiloti',
      baseAmount: 'Asosiy narx',
      addonsAmount: "Qo'shimcha xizmatlar",
      discount: 'Chegirma',
      extraDiscount: "Qo'shimcha chegirma",
      total: 'Jami',
      selectService: 'Xizmat turini tanlang',
      noServicesInCategory: "Bu yo'nalishda hozircha xizmat yo'q.",
      submit: 'Buyurtmani tasdiqlash',
      submitting: 'Yuborilmoqda…',
      loginToContinue: 'Davom etish uchun kiring',
      nextStepNote: "Keyingi bosqichda Payme yoki Click orqali onlayn to'lov qilasiz.",
      selectServiceError: 'Xizmat turini tanlang.',
      requiredFieldsError: "Iltimos, manzil, sana va telefon raqamini to'ldiring.",
      loadError: "Ma'lumotlarni yuklab bo'lmadi. Qaytadan urinib ko'ring.",
      submitError: "Buyurtma yuborilmadi. Qaytadan urinib ko'ring.",
    },
    login: {
      title: 'Tizimga kirish',
      heroQuote: "Onlayn band qiling, xavfsiz to'lang — tekshirilgan xizmatchimiz eshigingizga keladi.",
      email: 'Email',
      telegram: 'Telegram',
      emailLabel: 'Email',
      passwordLabel: 'Parol',
      loginButton: 'Kirish',
      loggingIn: 'Kirilmoqda…',
      telegramIntro:
        "Telegram bot orqali ro'yxatdan o'tgan telefon raqam va parolingizni kiriting — botga tasdiqlash kodi yuboriladi.",
      phoneLabel: 'Telefon raqam',
      sendCode: 'Telegramga kod yuborish',
      sendingCode: 'Yuborilmoqda…',
      notRegistered: "Hali ro'yxatdan o'tmaganmisiz? Telegram bot orqali ro'yxatdan o'ting",
      codeIntro: 'Telegram botga yuborilgan 6 xonali kodni kiriting.',
      codeLabel: 'Kod',
      verify: 'Tasdiqlash',
      verifying: 'Tekshirilmoqda…',
      back: 'Orqaga',
      noAccount: "Hisobingiz yo'qmi?",
      registerLink: "Ro'yxatdan o'ting",
      codeSendError: "Kod yuborilmadi. Qaytadan urinib ko'ring.",
      verifyError: "Tasdiqlanmadi. Qaytadan urinib ko'ring.",
      serviceUnavailable: 'Xizmat hozircha mavjud emas.',
    },
    register: {
      title: "Ro'yxatdan o'tish",
      heroQuote: "Bir necha daqiqada ro'yxatdan o'ting va birinchi buyurtmangizni band qiling.",
      fullNameLabel: "To'liq ism",
      phoneLabel: 'Telefon raqam',
      emailLabel: 'Email',
      passwordLabel: 'Parol',
      submit: "Ro'yxatdan o'tish",
      submitting: 'Yuborilmoqda…',
      successMessage: "Muvaffaqiyatli ro'yxatdan o'tdingiz! Kirish sahifasiga yo'naltirilmoqda…",
      telegramButton: "Telegram orqali ro'yxatdan o'tish",
      haveAccount: 'Hisobingiz bormi?',
      loginLink: 'Kirish',
      referralApplied: "Referral kodi qabul qilindi — birinchi buyurtmangizga -20% chegirma qo'llaniladi.",
    },
    dashboard: {
      greeting: 'Salom',
      defaultName: 'mijoz',
      subtitle: 'Buyurtmalaringiz tarixi va joriy holati.',
      myBookings: 'Buyurtmalarim',
      newBooking: '+ Yangi buyurtma',
      loading: 'Yuklanmoqda…',
      noBookings: "Hozircha buyurtmalar yo'q.",
      emptyTitle: 'Birinchi buyurtmangizni bering',
      emptyDesc: "Bir necha daqiqada onlayn band qiling — tekshirilgan xizmatchimiz belgilangan vaqtda eshigingizga keladi.",
      statusPending: "To'lov kutilmoqda",
      statusConfirmed: 'Tasdiqlangan',
      statusAssigned: 'Xizmatchi tayinlandi',
      statusInProgress: 'Bajarilmoqda',
      statusCompleted: 'Bajarildi',
      statusCancelled: 'Bekor qilindi',
      showcaseTitle: 'Yana xizmat kerakmi?',
      showcaseDesc: 'Bir necha bosim bilan yangi xizmat band qiling.',
      bookThis: 'Band qilish',
      referralTitle: "Do'stingizni taklif qiling",
      referralDesc: "Havolangiz orqali ro'yxatdan o'tgan do'stingiz birinchi buyurtmasiga -20% chegirma oladi. Uning buyurtmasi bajarilgach, siz kredit ishlab olasiz.",
      referralYourCode: 'Sizning kodingiz',
      referralCopyLink: 'Havolani nusxalash',
      referralCopied: 'Nusxalandi!',
      referralInvited: 'Taklif qilinganlar',
      referralCredits: 'Kredit balansi',
      referralCreditsNote: "Admin bilan bog'lanib, kreditingizni keyingi buyurtmangizga qo'lladiring.",
    },
    bookingDetail: {
      title: 'Buyurtma tafsilotlari',
      loading: 'Yuklanmoqda…',
      notFound: 'Buyurtma topilmadi.',
      service: 'Xizmat',
      tier: 'Tarif',
      address: 'Manzil',
      dateTime: 'Sana / vaqt',
      contact: 'Aloqa',
      total: 'Jami summa',
      assignedCleaner: 'Tayinlangan xizmatchi',
      cancelledMessage: 'Bu buyurtma bekor qilingan.',
      paidMessage: "To'lov muvaffaqiyatli amalga oshirildi. Xizmatchimiz belgilangan vaqtda tashrif buyuradi.",
      choosePayment: "To'lov usulini tanlang",
      gatewaysWarning:
        "Payme/Click merchant kalitlari hali sozlanmagan. Demo rejimda to'lov havolasi noto'g'ri bo'lishi mumkin.",
      payWithPayme: "Payme orqali to'lash",
      payWithClick: "Click orqali to'lash",
      redirecting: "Yo'naltirilmoqda…",
      orUploadReceipt: "Yoki to'lov chekini yuklang",
      uploadReceiptDesc:
        "Bank orqali to'g'ridan-to'g'ri to'lov qilgan bo'lsangiz, chek rasmini (yoki PDF) yuklang — administrator tekshirib, buyurtmani tasdiqlaydi.",
      receiptPending: 'Chek yuborildi, tekshirilmoqda.',
      viewUploaded: "Yuklangan faylni ko'rish",
      receiptRejected: "Oldingi chek rad etildi. Iltimos, to'g'ri chekni qaytadan yuklang.",
      uploadReceipt: 'Chekni yuklash',
      uploading: 'Yuklanmoqda…',
      uploadError: "Chekni yuklab bo'lmadi. Qaytadan urinib ko'ring.",
    },
    paymentResult: {
      checking: "To'lov holati tekshirilmoqda…",
      successTitle: "To'lov muvaffaqiyatli!",
      successDesc: 'Buyurtmangiz tasdiqlandi. Xizmatchimiz belgilangan vaqtda tashrif buyuradi.',
      pendingTitle: "To'lov kutilmoqda",
      pendingDesc:
        "To'lovingiz hali tasdiqlanmadi. Agar to'lov jarayonini yakunlagan bo'lsangiz, bir necha daqiqada holat yangilanadi.",
      viewBooking: "Buyurtmani ko'rish",
    },
  },

  en: {
    brand: {
      name: 'Prime Standard & Co',
      tagline: 'Cleaning Services',
      initials: 'PS',
    },
    nav: {
      home: 'Home',
      services: 'Services & Pricing',
      blog: 'Tips',
      about: 'About Us',
      contact: 'Contact',
      login: 'Log In',
      myAccount: 'My Account',
      signOut: 'Sign Out',
      bookNow: 'Book Now',
      adminPanel: 'Admin Panel',
      menu: 'Menu',
      toggleTheme: 'Toggle theme',
      language: 'Language',
    },
    footer: {
      tagline:
        'Reliable, fast and high-quality cleaning for your home or office. Book online — pay securely with Payme or Click.',
      servicesTitle: 'Services',
      services: ['Standard Cleaning', 'Deep Cleaning', 'Move In/Out Cleaning', 'Office Cleaning', 'Post-Construction Cleaning'],
      companyTitle: 'Company',
      about: 'About Us',
      blog: 'Tips',
      contact: 'Contact',
      bookNow: 'Book Now',
      contactTitle: 'Contact',
      address: 'Tashkent & Samarkand, Uzbekistan',
      rights: 'All rights reserved.',
      privacy: 'Privacy Policy',
      terms: 'Terms of Use',
    },
    discountBanner: {
      pre: '',
      percent: '-20% off your first order',
      mid: '— just add the code',
      code: 'YANGI20',
      post: 'in the booking notes.',
      cta: 'Book now',
      close: 'Close',
    },
    mobileBar: {
      call: 'Call',
      book: 'Book Now',
    },
    floatingContact: {
      whatsapp: 'Message us on WhatsApp',
      telegram: 'Message us on Telegram',
      aiChat: 'Chat with AI assistant',
      close: 'Close contact panel',
      open: 'Contact us',
      back: 'Back',
      aiTitle: 'AI Assistant',
      aiGreeting:
        "Hi! I'm Prime Standard & Co's AI assistant. Ask me anything about our services, prices, or booking.",
      aiUnavailable:
        'The AI assistant is unavailable right now. Message us directly on WhatsApp or by phone using the menu below.',
      aiPlaceholder: 'Type your question…',
      aiSending: 'Typing…',
      aiError: "Sorry, I couldn't reply. Please try again or reach us on WhatsApp.",
      aiSend: 'Send',
    },
    home: {
      heroTag: 'Cleaning service in Tashkent & Samarkand',
      heroTitle: 'Professional cleaning for your',
      heroTitleHighlight: 'home or office',
      heroDesc: 'Book online, pay securely with Payme or Click, and our vetted cleaner comes to your door.',
      ctaBook: 'Book Now',
      ctaPrices: 'View Prices',
      heroTrust1: 'Vetted cleaners',
      heroTrust2: 'Secure online payment',
      heroTrust3: 'Fast response',
      socialProofBadge: '4.9/5 (trusted by 1,200+ homes)',
      instantQuoteTitle: 'Instant Quote Calculator',
      roomsLabel: 'Number of rooms',
      typeLabel: 'Cleaning type',
      standardTitle: 'Standard',
      standardSub: 'Regular cleaning',
      deepTitle: 'Deep Clean',
      deepSub: 'Thorough cleaning',
      estimatedPriceLabel: 'Estimated price:',
      bookNowBtn: 'Book Now →',
      pillar1Title: 'Vetted Cleaners',
      pillar1Desc: '100% background-checked specialists',
      pillar2Title: 'Satisfaction Guarantee',
      pillar2Desc: 'If not satisfied, we re-clean for free',
      pillar3Title: 'Transparent Pricing',
      pillar3Desc: 'No hidden fees or surprise charges',
      resultsTag: 'Real Results',
      resultsTitle: 'Before & After Comparison',
      resultsDesc: 'Slide to see our cleaning quality transformation',
      beforeLabel: 'Before cleaning',
      afterLabel: 'After cleaning',
      plansTag: 'Transparent Plans',
      plansTitle: 'Choose Your Plan',
      plansDesc: 'Guaranteed prices with zero hidden commissions',
      priceFrom: '/from',
      selectBtn: 'Select',
      bookPlanBtn: 'Book Now',
      mostPopularBadge: '✨ MOST POPULAR',
      planStandardName: 'Standard Cleaning',
      planStandardDesc: 'Ideal for regular home maintenance',
      planStandardF1: 'Floor and carpet vacuuming',
      planStandardF2: 'Dusting surfaces & furniture',
      planStandardF3: 'Kitchen & bathroom sanitization',
      planDeepName: 'Deep Cleaning',
      planDeepDesc: 'Eliminate deep grime and tough stains',
      planDeepF1: 'All standard cleaning inclusions',
      planDeepF2: 'Behind furniture & hard-to-reach areas',
      planDeepF3: 'Tile grout & kitchen grease removal',
      planDeepF4: 'Window washing inside and outside',
      planMoveName: 'Move-In / Move-Out',
      planMoveDesc: 'For new home moves or post-renovation',
      planMoveF1: 'Full deep clean & disinfection',
      planMoveF2: 'Inside cabinet & drawer cleaning',
      planMoveF3: 'Post-construction debris cleanup',
      verifiedCustomer: 'Verified Customer',
      verifiedServiceMeta: 'Home cleaning · Tashkent',
      trustPoints: [
        { label: 'Vetted cleaners', desc: "Each one's ID/passport is checked, then personally interviewed" },
        { label: 'Secure online payment', desc: 'Via Payme and Click' },
        { label: 'Fast response', desc: 'We reply within minutes' },
        { label: 'Transparent pricing', desc: 'No hidden fees, known upfront' },
      ],
      reviewsCount: 'reviews',
      reviewsTitle: 'What our customers say',
      reviewsDesc: 'Real reviews from real customers — no fake or purchased reviews.',
      areaTitle: 'We serve Tashkent and Samarkand',
      areaDesc: 'Wherever your address is, our cleaner will arrive at the scheduled time.',
      referralTitle: 'Invite a friend — you both get a discount',
      referralDesc: 'When a friend signs up with your link, they get a discount on their first order, and you earn credit.',
      referralButton: 'Learn more',
      howTitle: 'How it works',
      howDesc: 'Get your home or office cleaned in three simple steps.',
      steps: [
        { title: '1. Choose a service', desc: 'Pick home or office, cleaning type, and any add-ons.' },
        { title: '2. Book a time', desc: 'Pick a date and time that works for you — one-off or recurring.' },
        { title: '3. Pay online', desc: 'Pay securely via Payme or Click and a cleaner is assigned.' },
      ],
      estimateTitle: 'Get an instant price estimate',
      estimateDesc: 'See an approximate price before you even sign up.',
      servicesTitle: 'Our Services',
      servicesDesc: 'Choose the service that fits your needs — price is calculated automatically at booking.',
      serviceTiles: [
        {
          title: 'Standard Cleaning',
          desc: 'Everyday maintenance — dusting, bathrooms, and kitchen.',
          img: 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=800&q=80',
        },
        {
          title: 'Deep Cleaning',
          desc: 'Move-in/move-out or seasonal deep clean.',
          img: 'https://images.unsplash.com/photo-1558317374-067fb5f30001?auto=format&fit=crop&w=800&q=80',
        },
        {
          title: 'Office Cleaning',
          desc: 'Regular or one-off cleaning for your workspace.',
          img: 'https://images.unsplash.com/photo-1527515637462-cff94eecc1ac?auto=format&fit=crop&w=800&q=80',
        },
        {
          title: 'Post-Construction Cleaning',
          desc: 'Complete post-renovation cleanup for new apartments and offices.',
          img: 'https://images.unsplash.com/photo-1628177142898-93e36e4e3a50?auto=format&fit=crop&w=800&q=80',
        },
      ],
      detail: 'Details',
      whyTag: 'Why Prime Standard & Co',
      whyTitle: 'Reliable, transparent and convenient',
      whyUs: [
        "Every cleaner's ID/passport is checked and their experience verified before joining the team",
        'Price is calculated up front based on room count or area — no hidden extras',
        'Payment only goes through verified, secure channels — Payme and Click',
        'Every booking status is tracked in real time from your personal account',
        'If our team damages your property during a visit, the company takes financial responsibility',
      ],
      teamTitle: 'Meet our team',
      teamDesc: 'Every cleaner has been personally interviewed and their experience verified.',
      teamExperience: 'yrs experience',
      teamSeeAll: 'See the full team →',
      galleryTitle: 'What the result looks like',
      galleryDesc: 'A clean, tidy room — the result you can expect from every visit.',
      faqTag: 'FAQ',
      faqTitle: 'Frequently Asked Questions',
      faqs: [
        {
          q: 'How are cleaners vetted?',
          a: 'Every cleaner is personally interviewed before being hired, and their experience and references are checked. Years of experience and customer rating are shown on their profile.',
        },
        {
          q: 'When do I pay?',
          a: 'After confirming your booking, you pay online via Payme or Click. Payment happens once a cleaner is assigned, before the service is performed.',
        },
        {
          q: "What if I'm not happy with the result?",
          a: 'Contact us via the Contact page or by phone — we work to resolve any issue quickly.',
        },
        {
          q: 'Can I cancel or reschedule a booking?',
          a: 'Yes, you can request to cancel or change the booking date from your personal account.',
        },
        {
          q: 'Which areas do you serve?',
          a: 'We currently serve the city of Tashkent. Enter your address on the booking form — if it\'s within our area, a cleaner will be assigned.',
        },
      ],
      ctaTitle: 'Book a cleaning today',
      ctaDesc: 'Get a price in seconds and pay online.',
      ctaButton: 'Book Now',
    },
    services: {
      heroTitle: 'Services & Pricing',
      heroDesc:
        "Our pricing is structured after leading US cleaning platforms (Merry Maids, Molly Maid, MaidPro) and adapted to the Uzbekistan market. The exact price, based on room count or area, is calculated automatically on the booking page.",
      loading: 'Loading…',
      home: 'Home',
      office: 'Office',
      startingPrice: 'starting price (1 room)',
      perExtraRoom: 'per additional room',
      min: 'min.',
      bookThis: 'Book this service',
      tariffsTitle: 'Tariff Tiers & Sample Pricing',
      tariffsDesc: 'Select the tier level that best matches your expectations for any order:',
      example1Room: 'Sample for a 1-room standard clean:',
      teamTitle: 'Our Team',
      teamDesc: 'Every one of our cleaners is vetted and tested.',
      yearsExperience: 'yrs experience',
    },
    about: {
      title: 'About Us',
      intro:
        'Prime Standard & Co is a modern cleaning service operating in Tashkent. Our goal is to let you get your home or office cleaned online in a few clicks, with vetted cleaners and secure online payment.',
      pillars: [
        { title: 'Vetted cleaners', desc: "ID/passport checked, then personally interviewed" },
        { title: 'Secure payment', desc: 'Via Payme, Click, or receipt' },
        { title: 'Eco-friendly supplies', desc: 'Cleaning products that are safe for your health' },
      ],
      whyTitle: 'Why choose us?',
      reasons: [
        'All cleaners pass an ID/passport check and a trial period before being hired',
        'Secure online payment via Payme and Click',
        'Transparent pricing — no hidden fees',
        'Eco-friendly, safe cleaning supplies',
        'We take financial responsibility for any damage caused during a service visit',
      ],
    },
    contact: {
      title: 'Contact',
      desc: "Have questions? Get in touch with us.",
      sentMessage: "Thanks for your message! We'll be in touch soon.",
      nameLabel: 'Name',
      contactLabel: 'Phone or email',
      messageLabel: 'Message',
      sending: 'Sending…',
      send: 'Send',
      error: 'Message failed to send. Please try again.',
      address: 'Tashkent, Uzbekistan',
    },
    privacy: {
      title: 'Privacy Policy',
      updated: 'Last updated: August 2026',
      sections: [
        {
          h: '1. What information we collect',
          p: "When you register or place a booking, we collect information such as your name, phone number, email and address. This information is used solely to provide the service — confirming your booking, assigning a cleaner, and contacting you.",
        },
        {
          h: '2. How we use your information',
          p: 'Your information is never sold to third parties. It is used only to fulfil your order (assigning a cleaner), process payment (Payme, Click), and to contact you when necessary.',
        },
        {
          h: '3. Data storage',
          p: 'Your data is stored on secure servers and is only accessed by authorized staff for the purpose of providing the service.',
        },
        {
          h: '4. Your rights',
          p: 'You can view your own data from your personal account. To request a change or deletion of your data, contact us at {email}.',
        },
        {
          h: '5. Cookies and technical data',
          p: 'For a smoother experience, your browser may store certain settings (such as your chosen theme and language). These are not used to identify you personally.',
        },
        {
          h: '6. Contact',
          p: 'If you have questions about this privacy policy, you can reach us at {email}.',
        },
      ],
    },
    terms: {
      title: 'Terms of Use',
      updated: 'Last updated: August 2026',
      sections: [
        {
          h: '1. About the service',
          p: 'Prime Standard & Co is an online booking platform for home and office cleaning services. By using this site, you agree to these terms.',
        },
        {
          h: '2. Orders and pricing',
          p: 'Price is calculated based on service type, room count or area, the selected tier, and any add-ons, and is shown up front during booking. The final price is confirmed before payment.',
        },
        {
          h: '3. Payment',
          p: 'Payments are made via Payme or Click. Once payment is successfully completed, the booking is considered confirmed.',
        },
        {
          h: '4. Cancellation and changes',
          p: 'To cancel or reschedule a booking, use your personal account or contact us by phone.',
        },
        {
          h: '5. Liability',
          p: "If you're not satisfied with the service, contact us via the Contact page — we work to resolve issues quickly.",
        },
        {
          h: '6. Changes to these terms',
          p: 'These terms may be updated from time to time. Significant changes will be announced on the site.',
        },
      ],
    },
    blog: {
      title: 'Tips',
      desc: 'Short, useful tips for keeping your home and office clean.',
      readMinutes: 'min read',
      read: 'Read',
      allPosts: 'All tips',
      ctaTitle: 'Need a cleaning service?',
      ctaDesc: 'Book online in just a few minutes.',
      ctaButton: 'Book Now',
    },
    notFound: {
      text: 'Page not found.',
      backHome: 'Back to home',
    },
    priceEstimator: {
      title: 'Price Estimate',
      desc: 'Choose a service type and size — see the price instantly.',
      serviceType: 'Select Service',
      area: 'Area (m²)',
      rooms: 'Number of rooms',
      estimateLabel: 'Estimated price (standard tier)',
      continue: 'Continue to booking',
    },
    pricing: {
      tierLabels: { standard: 'Standard', premium: 'Premium', elite: 'Elite' },
      tierPerks: {
        standard: ['Experienced cleaner', 'Standard cleaning supplies', 'Visit on the scheduled day'],
        premium: [
          'Highly experienced cleaner',
          'Premium eco-friendly supplies',
          'Extra quality check',
          'Priority time slot selection',
        ],
        elite: [
          'Top-rated team lead',
          'Premium supplies + disinfection',
          '2-stage quality control',
          'Exact one-hour time window',
          '24/7 priority customer support',
        ],
      },
      frequencyLabels: {
        once: 'One-time',
        weekly: 'Weekly (-20%)',
        biweekly: 'Bi-weekly (-15%)',
        monthly: 'Monthly (-10%)',
      },
      firstBookingDiscountLabel: 'First-order discount (-10%)',
      referralDiscountLabel: 'Referral discount (-20%)',
      repairTierLabels: { standard: 'Cosmetic', premium: 'Standard', elite: 'Euro Renovation' },
      repairTierPerks: {
        standard: [
          'Budget materials (you can also supply your own)',
          'Fast turnaround (1-2 weeks)',
          'Core work: painting, plastering, minor repairs',
        ],
        premium: [
          'Mid-grade quality materials',
          'Staged quality inspections',
          '1-year warranty',
          'Detailed estimate and timeline',
        ],
        elite: [
          'Premium imported materials',
          'Designer consultation + 3D project',
          'Dedicated project manager',
          '3-year warranty',
          'Free post-renovation cleaning included',
        ],
      },
    },
    booking: {
      title: 'Book a Service',
      subtitle: 'Fill in the details — the price is calculated automatically below.',
      loading: 'Loading…',
      firstBookingText: '20% first booking discount applied!',
      referralAppliedText: '15% referral discount applied!',
      categoryLabel: 'Category',
      categoryCleaning: 'Cleaning',
      categoryRepair: 'Repair',
      serviceType: 'Select Service',
      propertyType: 'Property Type',
      home: 'Home / Apartment',
      office: 'Office',
      area: 'Area (m²)',
      rooms: 'Number of Rooms',
      floor: 'Floor',
      floorHelp: 'The higher the floor, the higher the price (for lifting equipment/logistics)',
      address: 'Address',
      addressPlaceholder: 'Street, building, apartment number',
      city: 'City',
      frequency: 'Frequency',
      date: 'Date',
      time: 'Time',
      tier: 'Tier',
      repairTier: 'Renovation Grade',
      repairPromo: 'Free estimate and consultation — ask about our special price for first-time orders.',
      addonsLabel: 'Add-ons',
      repairPhotosLabel: 'Project photos (optional)',
      repairPhotosDesc: "Take a photo of the room's current condition for a more accurate quote. Up to 2 photos.",
      repairPhotosAdd: 'Add photo',
      repairPhotosRemove: 'Remove',
      repairNotesLabel: 'Project description (optional)',
      repairNotesPlaceholder: 'e.g. 2-room apartment, damp patches on the walls, needs new wiring...',
      repairPhotoError: 'Could not upload the photo. Please try again or choose a different one.',
      firstBookingBanner: "This is your first booking — a -10% discount has been automatically applied to the total.",
      referralBanner: 'You used a referral code — a -20% discount has been automatically applied to your first booking.',
      nameLabel: 'Name',
      phoneLabel: 'Phone Number',
      notesLabel: 'Additional notes (optional)',
      priceDetailsTitle: 'Price Details',
      baseAmount: 'Base price',
      addonsAmount: 'Add-ons',
      discount: 'Discount',
      extraDiscount: 'Extra discount',
      total: 'Total',
      selectService: 'Choose a service type',
      noServicesInCategory: 'No services in this category yet.',
      submit: 'Confirm Booking',
      submitting: 'Submitting…',
      loginToContinue: 'Log in to continue',
      nextStepNote: "In the next step you'll pay securely online via Payme or Click.",
      selectServiceError: 'Please choose a service type.',
      requiredFieldsError: 'Please fill in the address, date and phone number.',
      loadError: 'Could not load data. Please try again.',
      submitError: 'The booking could not be submitted. Please try again.',
    },
    login: {
      title: 'Log In',
      heroQuote: 'Book online, pay securely — our vetted cleaner comes to your door.',
      email: 'Email',
      telegram: 'Telegram',
      emailLabel: 'Email',
      passwordLabel: 'Password',
      loginButton: 'Log In',
      loggingIn: 'Logging in…',
      telegramIntro:
        'Enter the phone number and password you registered with via the Telegram bot — a confirmation code will be sent to it.',
      phoneLabel: 'Phone Number',
      sendCode: 'Send Code to Telegram',
      sendingCode: 'Sending…',
      notRegistered: "Don't have an account yet? Sign up via the Telegram bot",
      codeIntro: 'Enter the 6-digit code sent to your Telegram bot.',
      codeLabel: 'Code',
      verify: 'Verify',
      verifying: 'Verifying…',
      back: 'Back',
      noAccount: "Don't have an account?",
      registerLink: 'Sign up',
      codeSendError: 'Could not send the code. Please try again.',
      verifyError: 'Verification failed. Please try again.',
      serviceUnavailable: 'The service is currently unavailable.',
    },
    register: {
      title: 'Sign Up',
      heroQuote: 'Sign up in a few minutes and book your first cleaning.',
      fullNameLabel: 'Full Name',
      phoneLabel: 'Phone Number',
      emailLabel: 'Email',
      passwordLabel: 'Password',
      submit: 'Sign Up',
      submitting: 'Submitting…',
      successMessage: 'Successfully signed up! Redirecting to the login page…',
      telegramButton: 'Sign up via Telegram',
      haveAccount: 'Already have an account?',
      loginLink: 'Log in',
      referralApplied: 'Referral code accepted — a -20% discount will apply to your first booking.',
    },
    dashboard: {
      greeting: 'Hello',
      defaultName: 'customer',
      subtitle: 'Your booking history and current status.',
      myBookings: 'My Bookings',
      newBooking: '+ New Booking',
      loading: 'Loading…',
      noBookings: 'No bookings yet.',
      emptyTitle: 'Make your first booking',
      emptyDesc: 'Book online in just a few minutes — our vetted cleaner will arrive at your door at the scheduled time.',
      statusPending: 'Awaiting payment',
      statusConfirmed: 'Confirmed',
      statusAssigned: 'Cleaner assigned',
      statusInProgress: 'In progress',
      statusCompleted: 'Completed',
      statusCancelled: 'Cancelled',
      showcaseTitle: 'Need another service?',
      showcaseDesc: 'Book a new service in just a few clicks.',
      bookThis: 'Book',
      referralTitle: 'Invite a friend',
      referralDesc: 'A friend who signs up with your link gets -20% off their first booking. Once their booking is completed, you earn credit.',
      referralYourCode: 'Your code',
      referralCopyLink: 'Copy link',
      referralCopied: 'Copied!',
      referralInvited: 'Friends invited',
      referralCredits: 'Credit balance',
      referralCreditsNote: 'Contact us to apply your credit to your next booking.',
    },
    bookingDetail: {
      title: 'Booking Details',
      loading: 'Loading…',
      notFound: 'Booking not found.',
      service: 'Service',
      tier: 'Tier',
      address: 'Address',
      dateTime: 'Date / time',
      contact: 'Contact',
      total: 'Total amount',
      assignedCleaner: 'Assigned cleaner',
      cancelledMessage: 'This booking has been cancelled.',
      paidMessage: 'Payment successful. Our cleaner will visit at the scheduled time.',
      choosePayment: 'Choose a payment method',
      gatewaysWarning:
        'Payme/Click merchant keys are not configured yet. The payment link may not work correctly in demo mode.',
      payWithPayme: 'Pay with Payme',
      payWithClick: 'Pay with Click',
      redirecting: 'Redirecting…',
      orUploadReceipt: 'Or upload a payment receipt',
      uploadReceiptDesc:
        'If you paid directly via bank transfer, upload a photo (or PDF) of the receipt — an admin will review it and confirm the booking.',
      receiptPending: 'Receipt submitted, under review.',
      viewUploaded: 'View uploaded file',
      receiptRejected: 'The previous receipt was rejected. Please upload the correct receipt again.',
      uploadReceipt: 'Upload Receipt',
      uploading: 'Uploading…',
      uploadError: 'Could not upload the receipt. Please try again.',
    },
    paymentResult: {
      checking: 'Checking payment status…',
      successTitle: 'Payment Successful!',
      successDesc: 'Your booking is confirmed. Our cleaner will visit at the scheduled time.',
      pendingTitle: 'Payment Pending',
      pendingDesc: "Your payment hasn't been confirmed yet. If you completed the payment, the status will update within a few minutes.",
      viewBooking: 'View Booking',
    },
  },

  ru: {
    brand: {
      name: 'Prime Standard & Co',
      tagline: 'Клининговые услуги',
      initials: 'PS',
    },
    nav: {
      home: 'Главная',
      services: 'Услуги и цены',
      blog: 'Советы',
      about: 'О нас',
      contact: 'Контакты',
      login: 'Войти',
      myAccount: 'Личный кабинет',
      signOut: 'Выйти',
      bookNow: 'Заказать',
      adminPanel: 'Админ-панель',
      menu: 'Меню',
      toggleTheme: 'Сменить тему',
      language: 'Язык',
    },
    footer: {
      tagline:
        'Надёжная, быстрая и качественная уборка для вашего дома или офиса. Закажите онлайн — оплатите безопасно через Payme или Click.',
      servicesTitle: 'Услуги',
      services: ['Стандартная уборка', 'Генеральная уборка', 'Уборка при переезде', 'Уборка офисов', 'Уборка после строительства'],
      companyTitle: 'Компания',
      about: 'О нас',
      blog: 'Советы',
      contact: 'Контакты',
      bookNow: 'Заказать',
      contactTitle: 'Контакты',
      address: 'Ташкент и Самарканд, Узбекистан',
      rights: 'Все права защищены.',
      privacy: 'Политика конфиденциальности',
      terms: 'Условия использования',
    },
    discountBanner: {
      pre: '',
      percent: '-20% на первый заказ',
      mid: '— укажите промокод',
      code: 'YANGI20',
      post: 'в комментарии к заказу.',
      cta: 'Заказать сейчас',
      close: 'Закрыть',
    },
    mobileBar: {
      call: 'Позвонить',
      book: 'Заказать',
    },
    floatingContact: {
      whatsapp: 'Написать в WhatsApp',
      telegram: 'Написать в Telegram',
      aiChat: 'Чат с AI-помощником',
      close: 'Закрыть окно связи',
      open: 'Связаться с нами',
      back: 'Назад',
      aiTitle: 'AI-помощник',
      aiGreeting:
        'Здравствуйте! Я AI-помощник Prime Standard & Co. Если у вас есть вопросы об услугах, ценах или заказе — напишите.',
      aiUnavailable:
        'AI-помощник сейчас недоступен. Напишите нам напрямую в WhatsApp или по телефону — выберите вариант в меню ниже.',
      aiPlaceholder: 'Напишите ваш вопрос…',
      aiSending: 'Печатает…',
      aiError: 'Извините, не удалось ответить. Попробуйте снова или напишите нам в WhatsApp.',
      aiSend: 'Отправить',
    },
    home: {
      heroTag: 'Клининговые услуги в Ташкенте и Самарканде',
      heroTitle: 'Профессиональная уборка для вашего',
      heroTitleHighlight: 'дома или офиса',
      heroDesc:
        'Закажите онлайн, оплатите безопасно через Payme или Click, и наш проверенный клинер приедет к вам.',
      ctaBook: 'Заказать сейчас',
      ctaPrices: 'Смотреть цены',
      heroTrust1: 'Проверенные клинеры',
      heroTrust2: 'Безопасная онлайн-оплата',
      heroTrust3: 'Быстрый ответ',
      socialProofBadge: '4.9/5 (доверие 1,200+ семей)',
      instantQuoteTitle: 'Быстрый расчет стоимости',
      roomsLabel: 'Количество комнат',
      typeLabel: 'Тип уборки',
      standardTitle: 'Стандарт',
      standardSub: 'Регулярная уборка',
      deepTitle: 'Генеральная',
      deepSub: 'Глубокая уборка',
      estimatedPriceLabel: 'Примерная цена:',
      bookNowBtn: 'Забронировать →',
      pillar1Title: 'Проверенные сотрудники',
      pillar1Desc: '100% проверенные специалисты',
      pillar2Title: 'Гарантия качества',
      pillar2Desc: 'Если не понравится, переуберем бесплатно',
      pillar3Title: 'Прозрачные цены',
      pillar3Desc: 'Без скрытых комиссий и переплат',
      resultsTag: 'Реальные результаты',
      resultsTitle: 'Сравнение До и После',
      resultsDesc: 'Потяните слайдер, чтобы оценить качество уборки',
      beforeLabel: 'До уборки',
      afterLabel: 'После уборки',
      plansTag: 'Прозрачные тарифы',
      plansTitle: 'Выберите подходящий тариф',
      plansDesc: 'Гарантированные цены без скрытых комиссий',
      priceFrom: '/от',
      selectBtn: 'Выбрать',
      bookPlanBtn: 'Забронировать сейчас',
      mostPopularBadge: '✨ САМЫЙ ПОПУЛЯРНЫЙ',
      planStandardName: 'Стандартная уборка',
      planStandardDesc: 'Идеально для поддержания чистоты',
      planStandardF1: 'Мытье полов и пылесошение ковров',
      planStandardF2: 'Протирание пыли с поверхностей',
      planStandardF3: 'Дезинфекция кухни и ванной',
      planDeepName: 'Генеральная уборка',
      planDeepDesc: 'Удаление глубоких загрязнений и въевшихся пятен',
      planDeepF1: 'Все услуги стандартной уборки',
      planDeepF2: 'За мебелью и труднодоступные места',
      planDeepF3: 'Швы кафеля и жировые пятна',
      planDeepF4: 'Мытье окон изнутри и снаружи',
      planMoveName: 'Уборка при переезде',
      planMoveDesc: 'После ремонта или при въезде/выезде',
      planMoveF1: 'Полная дезинфекция и генеральная уборка',
      planMoveF2: 'Мытье шкафов и ящиков изнутри',
      planMoveF3: 'Удаление следов ремонта и краски',
      verifiedCustomer: 'Проверенный клиент',
      verifiedServiceMeta: 'Уборка квартиры · Ташкент',
      trustPoints: [
        { label: 'Проверенные клинеры', desc: 'У каждого проверяется документ/паспорт, затем — личное собеседование' },
        { label: 'Безопасная онлайн-оплата', desc: 'Через Payme и Click' },
        { label: 'Быстрый ответ', desc: 'Отвечаем в течение нескольких минут' },
        { label: 'Прозрачные цены', desc: 'Без скрытых платежей, известны заранее' },
      ],
      reviewsCount: 'отзывов',
      reviewsTitle: 'Отзывы наших клиентов',
      reviewsDesc: 'Настоящие отзывы от настоящих клиентов — без фальшивых или купленных отзывов.',
      areaTitle: 'Работаем по Ташкенту и Самарканду',
      areaDesc: 'В каком бы районе ни был ваш адрес, наш клинер приедет в назначенное время.',
      referralTitle: 'Пригласите друга — скидку получите оба',
      referralDesc: 'Если друг зарегистрируется по вашей ссылке, он получит скидку на первый заказ, а вы заработаете кредит.',
      referralButton: 'Подробнее',
      howTitle: 'Как это работает',
      howDesc: 'Закажите уборку дома или офиса всего за три простых шага.',
      steps: [
        { title: '1. Выберите услугу', desc: 'Дом или офис, тип уборки и дополнительные услуги.' },
        { title: '2. Забронируйте время', desc: 'Выберите удобную дату и время — разово или регулярно.' },
        { title: '3. Оплатите онлайн', desc: 'Оплатите безопасно через Payme или Click, вам назначат клинера.' },
      ],
      estimateTitle: 'Узнайте цену прямо сейчас',
      estimateDesc: 'Посмотрите приблизительную цену ещё до регистрации.',
      servicesTitle: 'Наши услуги',
      servicesDesc: 'Выберите подходящую услугу — цена рассчитывается автоматически при бронировании.',
      serviceTiles: [
        {
          title: 'Стандартная уборка',
          desc: 'Повседневная чистота — уборка пыли, санузел и кухня.',
          img: 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=800&q=80',
        },
        {
          title: 'Генеральная уборка',
          desc: 'Уборка при переезде или сезонная генеральная уборка.',
          img: 'https://images.unsplash.com/photo-1558317374-067fb5f30001?auto=format&fit=crop&w=800&q=80',
        },
        {
          title: 'Уборка офисов',
          desc: 'Регулярная или разовая уборка вашего рабочего места.',
          img: 'https://images.unsplash.com/photo-1527515637462-cff94eecc1ac?auto=format&fit=crop&w=800&q=80',
        },
        {
          title: 'Уборка после строительства',
          desc: 'Полная уборка после ремонта для новостроек и офисов.',
          img: 'https://images.unsplash.com/photo-1628177142898-93e36e4e3a50?auto=format&fit=crop&w=800&q=80',
        },
      ],
      detail: 'Подробнее',
      whyTag: 'Почему Prime Standard & Co',
      whyTitle: 'Надёжно, прозрачно и удобно',
      whyUs: [
        'У каждого клинера проверяется документ/паспорт, а его опыт проверяется до начала работы',
        'Цена рассчитывается заранее по количеству комнат или площади — без скрытых доплат',
        'Оплата проходит только через проверенные безопасные каналы — Payme и Click',
        'Статус каждого заказа отслеживается в реальном времени в личном кабинете',
        'Если во время визита нашей команды был нанесён ущерб вашему имуществу, компания берёт на себя материальную ответственность',
      ],
      teamTitle: 'Познакомьтесь с нашей командой',
      teamDesc: 'Каждый наш клинер лично прошёл собеседование, а его опыт проверен.',
      teamExperience: 'лет опыта',
      teamSeeAll: 'Смотреть всю команду →',
      galleryTitle: 'Как выглядит результат',
      galleryDesc: 'Чистая и аккуратная комната — результат, который вы получите после каждого визита.',
      faqTag: 'Вопрос-ответ',
      faqTitle: 'Часто задаваемые вопросы',
      faqs: [
        {
          q: 'Как проверяются клинеры?',
          a: 'Каждый клинер лично проходит собеседование перед приёмом на работу, его опыт и рекомендации проверяются. В профиле указаны стаж работы и рейтинг клиентов.',
        },
        {
          q: 'Когда нужно платить?',
          a: 'После подтверждения заказа вы оплачиваете онлайн через Payme или Click. Оплата производится после назначения клинера, до оказания услуги.',
        },
        {
          q: 'Что делать, если я не доволен результатом?',
          a: 'Свяжитесь с нами через страницу контактов или по телефону — мы стараемся быстро решить любую проблему.',
        },
        {
          q: 'Можно ли отменить или перенести заказ?',
          a: 'Да, вы можете отправить запрос на отмену или изменение даты заказа из личного кабинета.',
        },
        {
          q: 'В каких районах вы работаете?',
          a: 'Пока мы работаем по городу Ташкенту. Укажите ваш адрес в форме бронирования — если район подходит, вам назначат клинера.',
        },
      ],
      ctaTitle: 'Закажите уборку сегодня',
      ctaDesc: 'Узнайте цену за несколько секунд и оплатите онлайн.',
      ctaButton: 'Заказать',
    },
    services: {
      heroTitle: 'Услуги и цены',
      heroDesc:
        'Наши цены сформированы на основе структуры ведущих клининговых платформ США (Merry Maids, Molly Maid, MaidPro) и адаптированы под рынок Узбекистана. Точная цена по количеству комнат или площади рассчитывается автоматически на странице бронирования.',
      loading: 'Загрузка…',
      home: 'Дом',
      office: 'Офис',
      startingPrice: 'начальная цена (1 комната)',
      perExtraRoom: 'за каждую дополнительную комнату',
      min: 'мин.',
      bookThis: 'Заказать эту услугу',
      tariffsTitle: 'Уровни тарифов и примерные цены',
      tariffsDesc: 'Выберите подходящий уровень тарифа под ваши потребности:',
      example1Room: 'Пример для 1-комнатной стандартной уборки:',
      teamTitle: 'Наша команда',
      teamDesc: 'Каждый наш клинер проверен и прошёл испытание.',
      yearsExperience: 'лет опыта',
    },
    about: {
      title: 'О нас',
      intro:
        'Prime Standard & Co — современный клининговый сервис, работающий в Ташкенте. Наша цель — дать вам возможность заказать уборку дома или офиса онлайн в несколько кликов, с проверенными клинерами и безопасной онлайн-оплатой.',
      pillars: [
        { title: 'Проверенные клинеры', desc: 'Проверка документа/паспорта, затем личное собеседование' },
        { title: 'Безопасная оплата', desc: 'Через Payme, Click или по чеку' },
        { title: 'Экологичные средства', desc: 'Безопасные для здоровья чистящие средства' },
      ],
      whyTitle: 'Почему именно мы?',
      reasons: [
        'Все клинеры проходят проверку документа/паспорта и испытательный срок перед приёмом на работу',
        'Безопасная онлайн-оплата через Payme и Click',
        'Прозрачное ценообразование — без скрытых платежей',
        'Экологичные и безопасные чистящие средства',
        'Берём на себя материальную ответственность за ущерб, нанесённый во время оказания услуги',
      ],
    },
    contact: {
      title: 'Контакты',
      desc: 'Есть вопросы? Свяжитесь с нами.',
      sentMessage: 'Спасибо за сообщение! Мы скоро свяжемся с вами.',
      nameLabel: 'Имя',
      contactLabel: 'Телефон или email',
      messageLabel: 'Сообщение',
      sending: 'Отправка…',
      send: 'Отправить',
      error: 'Не удалось отправить сообщение. Попробуйте снова.',
      address: 'Ташкент, Узбекистан',
    },
    privacy: {
      title: 'Политика конфиденциальности',
      updated: 'Последнее обновление: август 2026 г.',
      sections: [
        {
          h: '1. Какую информацию мы собираем',
          p: 'При регистрации или оформлении заказа мы собираем такую информацию, как имя, номер телефона, email и адрес. Эта информация используется исключительно для оказания услуги — подтверждения заказа, назначения клинера и связи с вами.',
        },
        {
          h: '2. Как мы используем информацию',
          p: 'Ваши данные никогда не продаются третьим лицам. Они используются только для выполнения заказа (назначение клинера), обработки платежа (Payme, Click) и связи с вами при необходимости.',
        },
        {
          h: '3. Хранение данных',
          p: 'Данные хранятся на защищённых серверах и доступны только уполномоченным сотрудникам в целях оказания услуги.',
        },
        {
          h: '4. Ваши права',
          p: 'Вы можете просматривать свои данные в личном кабинете. Чтобы запросить изменение или удаление данных, свяжитесь с нами по адресу {email}.',
        },
        {
          h: '5. Cookie и технические данные',
          p: 'Для удобства использования сайта в вашем браузере могут сохраняться некоторые настройки (например, выбранная тема и язык). Они не используются для идентификации вашей личности.',
        },
        {
          h: '6. Связь с нами',
          p: 'Если у вас есть вопросы по политике конфиденциальности, вы можете связаться с нами по адресу {email}.',
        },
      ],
    },
    terms: {
      title: 'Условия использования',
      updated: 'Последнее обновление: август 2026 г.',
      sections: [
        {
          h: '1. Об услуге',
          p: 'Prime Standard & Co — платформа для онлайн-бронирования клининговых услуг для дома и офиса. Используя сайт, вы соглашаетесь с данными условиями.',
        },
        {
          h: '2. Заказ и цены',
          p: 'Цена рассчитывается в зависимости от типа услуги, количества комнат или площади, выбранного тарифа и дополнительных услуг, и показывается заранее в процессе бронирования. Итоговая цена подтверждается перед оплатой.',
        },
        {
          h: '3. Оплата',
          p: 'Оплата производится через Payme или Click. После успешной оплаты заказ считается подтверждённым.',
        },
        {
          h: '4. Отмена и изменение',
          p: 'Для отмены заказа или изменения даты обратитесь через личный кабинет или свяжитесь с нами по телефону.',
        },
        {
          h: '5. Ответственность',
          p: 'Если вы не удовлетворены результатом услуги, обратитесь через страницу контактов — мы стараемся быстро решить проблему.',
        },
        {
          h: '6. Изменение условий',
          p: 'Данные условия могут периодически обновляться. О существенных изменениях будет объявлено на сайте.',
        },
      ],
    },
    blog: {
      title: 'Советы',
      desc: 'Короткие и полезные советы о том, как поддерживать чистоту дома и офиса.',
      readMinutes: 'мин чтения',
      read: 'Читать',
      allPosts: 'Все советы',
      ctaTitle: 'Нужна уборка?',
      ctaDesc: 'Забронируйте онлайн за несколько минут.',
      ctaButton: 'Заказать',
    },
    notFound: {
      text: 'Страница не найдена.',
      backHome: 'Вернуться на главную',
    },
    priceEstimator: {
      title: 'Примерная цена',
      desc: 'Выберите тип услуги и размер — цена появится мгновенно.',
      serviceType: 'Выберите услугу',
      area: 'Площадь (м²)',
      rooms: 'Количество комнат',
      estimateLabel: 'Примерная цена (стандартный тариф)',
      continue: 'Продолжить бронирование',
    },
    pricing: {
      tierLabels: { standard: 'Стандарт', premium: 'Премиум', elite: 'Элит' },
      tierPerks: {
        standard: ['Опытный клинер', 'Стандартные чистящие средства', 'Визит в назначенный день'],
        premium: [
          'Клинер с большим опытом',
          'Премиальные экологичные средства',
          'Дополнительный контроль качества',
          'Приоритет при выборе времени',
        ],
        elite: [
          'Руководитель команды с высшим рейтингом',
          'Премиальные средства + дезинфекция',
          '2-этапный контроль качества',
          'Точное часовое окно времени',
          'Приоритетная поддержка 24/7',
        ],
      },
      frequencyLabels: {
        once: 'Разово',
        weekly: 'Каждую неделю (-20%)',
        biweekly: 'Раз в две недели (-15%)',
        monthly: 'Раз в месяц (-10%)',
      },
      firstBookingDiscountLabel: 'Скидка на первый заказ (-10%)',
      referralDiscountLabel: 'Реферальная скидка (-20%)',
      repairTierLabels: { standard: 'Косметический', premium: 'Стандарт', elite: 'Евроремонт' },
      repairTierPerks: {
        standard: [
          'Бюджетные материалы (можно предоставить свои)',
          'Быстрое выполнение (1-2 недели)',
          'Основные работы: покраска, шпаклёвка, мелкий ремонт',
        ],
        premium: [
          'Материалы среднего класса',
          'Поэтапный контроль качества',
          'Гарантия 1 год',
          'Подробная смета и график работ',
        ],
        elite: [
          'Премиальные импортные материалы',
          'Консультация дизайнера + 3D-проект',
          'Персональный менеджер проекта',
          'Гарантия 3 года',
          'Бесплатная уборка после ремонта',
        ],
      },
    },
    booking: {
      title: 'Забронировать услугу',
      subtitle: 'Заполните данные — цена рассчитывается автоматически ниже.',
      loading: 'Загрузка…',
      firstBookingText: 'Скидка 20% на первый заказ применена!',
      referralAppliedText: 'Скидка 15% по реферальному коду применена!',
      categoryLabel: 'Направление',
      categoryCleaning: 'Уборка',
      categoryRepair: 'Ремонт',
      serviceType: 'Выберите услугу',
      propertyType: 'Тип объекта',
      home: 'Дом / квартира',
      office: 'Офис',
      area: 'Площадь (м²)',
      rooms: 'Количество комнат',
      floor: 'Этаж',
      floorHelp: 'Чем выше этаж, тем выше цена (за подъём оборудования/логистику)',
      address: 'Адрес',
      addressPlaceholder: 'Улица, дом, номер квартиры',
      city: 'Город',
      frequency: 'Частота',
      date: 'Дата',
      time: 'Время',
      tier: 'Тариф',
      repairTier: 'Уровень ремонта',
      repairPromo: 'Бесплатная смета и консультация — уточните специальную цену для первого заказа.',
      addonsLabel: 'Дополнительные услуги',
      repairPhotosLabel: 'Фото проекта (необязательно)',
      repairPhotosDesc: 'Сфотографируйте текущее состояние помещения — так мы дадим более точную оценку. Максимум 2 фото.',
      repairPhotosAdd: 'Добавить фото',
      repairPhotosRemove: 'Удалить',
      repairNotesLabel: 'Описание проекта (необязательно)',
      repairNotesPlaceholder: 'Например: 2-комнатная квартира, на стенах пятна сырости, нужна новая проводка...',
      repairPhotoError: 'Не удалось загрузить фото. Попробуйте снова или выберите другое.',
      firstBookingBanner: 'Это ваш первый заказ — к итоговой сумме автоматически применена скидка -10%.',
      referralBanner: 'Вы использовали реферальный код — к вашему первому заказу автоматически применена скидка -20%.',
      nameLabel: 'Имя',
      phoneLabel: 'Номер телефона',
      notesLabel: 'Дополнительный комментарий (необязательно)',
      priceDetailsTitle: 'Детали цены',
      baseAmount: 'Базовая цена',
      addonsAmount: 'Дополнительные услуги',
      discount: 'Скидка',
      extraDiscount: 'Дополнительная скидка',
      total: 'Итого',
      selectService: 'Выберите тип услуги',
      noServicesInCategory: 'В этом направлении пока нет услуг.',
      submit: 'Подтвердить заказ',
      submitting: 'Отправка…',
      loginToContinue: 'Войдите, чтобы продолжить',
      nextStepNote: 'На следующем шаге вы безопасно оплатите онлайн через Payme или Click.',
      selectServiceError: 'Выберите тип услуги.',
      requiredFieldsError: 'Пожалуйста, заполните адрес, дату и номер телефона.',
      loadError: 'Не удалось загрузить данные. Попробуйте снова.',
      submitError: 'Не удалось отправить заказ. Попробуйте снова.',
    },
    login: {
      title: 'Войти',
      heroQuote: 'Закажите онлайн, оплатите безопасно — наш проверенный клинер приедет к вам.',
      email: 'Email',
      telegram: 'Telegram',
      emailLabel: 'Email',
      passwordLabel: 'Пароль',
      loginButton: 'Войти',
      loggingIn: 'Вход…',
      telegramIntro:
        'Введите номер телефона и пароль, указанные при регистрации через Telegram-бота — на него будет отправлен код подтверждения.',
      phoneLabel: 'Номер телефона',
      sendCode: 'Отправить код в Telegram',
      sendingCode: 'Отправка…',
      notRegistered: 'Ещё не зарегистрированы? Зарегистрируйтесь через Telegram-бота',
      codeIntro: 'Введите 6-значный код, отправленный в Telegram-бота.',
      codeLabel: 'Код',
      verify: 'Подтвердить',
      verifying: 'Проверка…',
      back: 'Назад',
      noAccount: 'Нет аккаунта?',
      registerLink: 'Зарегистрироваться',
      codeSendError: 'Не удалось отправить код. Попробуйте снова.',
      verifyError: 'Не удалось подтвердить. Попробуйте снова.',
      serviceUnavailable: 'Сервис временно недоступен.',
    },
    register: {
      title: 'Регистрация',
      heroQuote: 'Зарегистрируйтесь за пару минут и закажите первую уборку.',
      fullNameLabel: 'Полное имя',
      phoneLabel: 'Номер телефона',
      emailLabel: 'Email',
      passwordLabel: 'Пароль',
      submit: 'Зарегистрироваться',
      submitting: 'Отправка…',
      successMessage: 'Регистрация прошла успешно! Перенаправление на страницу входа…',
      telegramButton: 'Зарегистрироваться через Telegram',
      haveAccount: 'Уже есть аккаунт?',
      loginLink: 'Войти',
      referralApplied: 'Реферальный код принят — скидка -20% будет применена к вашему первому заказу.',
    },
    dashboard: {
      greeting: 'Здравствуйте',
      defaultName: 'клиент',
      subtitle: 'История и текущий статус ваших заказов.',
      myBookings: 'Мои заказы',
      newBooking: '+ Новый заказ',
      loading: 'Загрузка…',
      noBookings: 'Пока нет заказов.',
      emptyTitle: 'Оформите первый заказ',
      emptyDesc: 'Забронируйте онлайн за пару минут — проверенный клинер приедет к вам в назначенное время.',
      statusPending: 'Ожидание оплаты',
      statusConfirmed: 'Подтверждён',
      statusAssigned: 'Клинер назначен',
      statusInProgress: 'Выполняется',
      statusCompleted: 'Выполнено',
      statusCancelled: 'Отменён',
      showcaseTitle: 'Нужна ещё услуга?',
      showcaseDesc: 'Закажите новую услугу в несколько кликов.',
      bookThis: 'Заказать',
      referralTitle: 'Пригласите друга',
      referralDesc: 'Друг, который зарегистрируется по вашей ссылке, получит -20% на первый заказ. Когда его заказ будет выполнен, вы заработаете кредит.',
      referralYourCode: 'Ваш код',
      referralCopyLink: 'Скопировать ссылку',
      referralCopied: 'Скопировано!',
      referralInvited: 'Приглашено друзей',
      referralCredits: 'Баланс кредита',
      referralCreditsNote: 'Свяжитесь с нами, чтобы применить кредит к следующему заказу.',
    },
    bookingDetail: {
      title: 'Детали заказа',
      loading: 'Загрузка…',
      notFound: 'Заказ не найден.',
      service: 'Услуга',
      tier: 'Тариф',
      address: 'Адрес',
      dateTime: 'Дата / время',
      contact: 'Контакт',
      total: 'Итоговая сумма',
      assignedCleaner: 'Назначенный клинер',
      cancelledMessage: 'Этот заказ отменён.',
      paidMessage: 'Оплата прошла успешно. Наш клинер приедет в назначенное время.',
      choosePayment: 'Выберите способ оплаты',
      gatewaysWarning:
        'Мерчант-ключи Payme/Click ещё не настроены. В демо-режиме ссылка на оплату может быть некорректной.',
      payWithPayme: 'Оплатить через Payme',
      payWithClick: 'Оплатить через Click',
      redirecting: 'Перенаправление…',
      orUploadReceipt: 'Или загрузите чек об оплате',
      uploadReceiptDesc:
        'Если вы оплатили напрямую банковским переводом, загрузите фото (или PDF) чека — администратор проверит и подтвердит заказ.',
      receiptPending: 'Чек отправлен, на проверке.',
      viewUploaded: 'Посмотреть загруженный файл',
      receiptRejected: 'Предыдущий чек был отклонён. Пожалуйста, загрузите корректный чек ещё раз.',
      uploadReceipt: 'Загрузить чек',
      uploading: 'Загрузка…',
      uploadError: 'Не удалось загрузить чек. Попробуйте снова.',
    },
    paymentResult: {
      checking: 'Проверка статуса оплаты…',
      successTitle: 'Оплата прошла успешно!',
      successDesc: 'Ваш заказ подтверждён. Наш клинер приедет в назначенное время.',
      pendingTitle: 'Ожидание оплаты',
      pendingDesc: 'Ваш платёж пока не подтверждён. Если вы завершили оплату, статус обновится в течение нескольких минут.',
      viewBooking: 'Посмотреть заказ',
    },
  },
} as const

export default translations
