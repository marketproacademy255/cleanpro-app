...
export default function AdminStaff() {
  ...
  async function load() {
    const data = await apiFetch<Cleaner[]>('admin-staff').catch(() => [])
    setCleaners(data)
    setLoading(false)
  }
  ...
  async function addCleaner(e: FormEvent) {
    ...
    setSaving(true)
    await apiFetch('admin-staff', { method: 'POST', body: JSON.stringify(form) }).catch(() => null)
    setForm(emptyForm)
    setSaving(false)
    load()
  }
  ...
  async function toggleActive(c: Cleaner) {
    await apiFetch(`admin-staff?id=${c.id}`, {
      method: 'PATCH',
