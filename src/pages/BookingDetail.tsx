...
export default function BookingDetail() {
  ...
  if (!booking) return <div className="section py-20 text-center text-gray-400 dark:text-gray-500">{t('bookingDetail.notFound')}</div>
    ...
    <div className="section max-w-2xl py-14">
      ...
      {booking.cleaners && (
        <div className="card mt-6">
          <h3 className="mb-4 text-sm font-semibold text-gray-900 dark:text-gray-100">{t('bookingDetail.assignedCleaner')}</h3>
          <div className="flex items-center gap-4">
            <div className="grid h-16 w-16 shrink-0 place-items-center rounded-full bg-brand-100 text-xl font-bold text-brand-700 dark:bg-brand-900/40 dark:text-brand-300">
              {booking.cleaners.full_name.charAt(0)}
            </div>
            <div>
              <div className="font-semibold text-gray-900 dark:text-gray-100">{booking.cleaners.full_name}</div>
              <div className="mt-1">
      ...
      <div className="card mt-6 space-y-2 text-sm">
        ...
        <Row label={t('bookingDetail.total')} value={<span className="text-lg font-bold text-gray-900 dark:text-gray-100">{formatUZS(booking.total_amount)}</span>} />
      </div>

      {isPaid ? (
        <div className="card mt-6 flex items-start gap-2.5 border-brand-200 bg-brand-50 text-brand-700 dark:border-brand-900/50 dark:bg-brand-900/20 dark:text-brand-300">
...
function StatusStepper({ status }: { status: BookingStatus }) {
  ...
  if (status === 'cancelled') {
    return (
      <div className="card mt-6 flex items-start gap-2.5 border-red-200 bg-red-50 text-red-600 dark:border-red-900/50 dark:bg-red-900/20 dark:text-red-300">
            ...
            <div key={step} className="flex flex-1 items-start last:flex-none">
              <div className="flex w-16 flex-col items-center gap-2 sm:w-20">
                <motion.div
                  ...
                  transition={isCurrent ? { repeat: Infinity, duration: 2 } : { duration: 0.2 }}
                ...
                <span
                  className={`text-center text-[11px] font-medium leading-tight transition-colors ${
                    isDone ? 'text-brand-700 dark:text-brand-400' : 'text-gray-400 dark:text-gray-500'
                  }`}
                >
                  {meta[step].label}
                </span>
              ...
              {i < STATUS_STEPS.length - 1 && (
                <div className="relative mt-4 h-0.5 flex-1 bg-gray-200 dark:bg-gray-700">
                  <motion.div
